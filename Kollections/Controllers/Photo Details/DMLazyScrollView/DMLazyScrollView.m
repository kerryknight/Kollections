...
//knightka added these two setContentOffset b/c this motherfucking class sucks and i never should've started
//with it but now i've already spent 6 hours fucking with it so i'm determined to get it working
...
- (KKPhotoDetailsTableViewController *) visibleViewController {
...
- (KKPhotoDetailsTableViewController *) viewControllerFromView:(UIView*) targetView {
    return (KKPhotoDetailsTableViewController *)[self traverseResponderChainForUIViewController:targetView];
}

- (id) traverseResponderChainForUIViewController:(UIView *) targetView {
    id nextResponder = [targetView nextResponder];
    if ([nextResponder isKindOfClass:[KKPhotoDetailsTableViewController class]]) {
...
- (KKPhotoDetailsTableViewController *) loadControllerAtIndex:(NSInteger) index andPlaceAtIndex:(NSInteger) destIndex {
    
    KKPhotoDetailsTableViewController *viewController = dataSource(index);
