...
@interface KKPhotoDetailsHeaderView ()
...
@implementation KKPhotoDetailsHeaderView
...
- (id)initWithFrame:(CGRect)frame photo:(PFObject*)aPhoto {
    ...
    if (self) {
        ...
        self.photo = aPhoto;
        self.photographer = [self.photo objectForKey:kKKPhotoUserKey];
        self.likeUsers = nil;
...
- (void)drawRect:(CGRect)rect {
    [super drawRect:rect];
    [KKUtility drawSideDropShadowForRect:self.nameHeaderView.frame inContext:UIGraphicsGetCurrentContext()];
    [KKUtility drawSideDropShadowForRect:self.photoImageView.frame inContext:UIGraphicsGetCurrentContext()];
    [KKUtility drawSideDropShadowForRect:self.likeBarView.frame inContext:UIGraphicsGetCurrentContext()];
}


#pragma mark - KKPhotoDetailsHeaderView
...
- (void)setLikeUsers:(NSMutableArray *)anArray {
    likeUsers = [anArray sortedArrayUsingComparator:^NSComparisonResult(PFUser *liker1, PFUser *liker2) {
        NSString *displayName1 = [liker1 objectForKey:kKKUserDisplayNameKey];
        NSString *displayName2 = [liker2 objectForKey:kKKUserDisplayNameKey];
    ...
    for (KKProfileImageView *image in currentLikeAvatars) {
    ...
    for (i = 0; i < numOfPics; i++) {
        KKProfileImageView *profilePic = [[KKProfileImageView alloc] init];
        ...
        [profilePic setFile:[[self.likeUsers objectAtIndex:i] objectForKey:kKKUserProfilePicSmallKey]];
...
- (void)reloadLikeBar {
    self.likeUsers = [[KKCache sharedCache] likersForPhoto:self.photo];
    [self setLikeButtonState:[[KKCache sharedCache] isPhotoLikedByCurrentUser:self.photo]];
...
- (void)createView {    
    ...
    PFFile *imageFile = [self.photo objectForKey:kKKPhotoPictureKey];
    ...
    [self.photographer fetchIfNeededInBackgroundWithBlock:^(PFObject *object, NSError *error) {
        // Create avatar view
        KKProfileImageView *avatarImageView = [[KKProfileImageView alloc] initWithFrame:CGRectMake(avatarImageX, avatarImageY, avatarImageDim, avatarImageDim)];
        [avatarImageView setFile:[self.photographer objectForKey:kKKUserProfilePicSmallKey]];
        ...
        // Create name label
        NSString *nameString = [self.photographer objectForKey:kKKUserDisplayNameKey];
...
- (void)didTapLikePhotoButtonAction:(UIButton *)button {
    ...
    if (liked) {
        [[KKCache sharedCache] incrementLikerCountForPhoto:self.photo];
        [newLikeUsersSet addObject:[PFUser currentUser]];
    } else {
        [[KKCache sharedCache] decrementLikerCountForPhoto:self.photo];
    }
    
    [[KKCache sharedCache] setPhotoIsLikedByCurrentUser:self.photo liked:liked];

    [self setLikeUsers:[newLikeUsersSet allObjects]];

    if (liked) {
        [KKUtility likePhotoInBackground:self.photo block:^(BOOL succeeded, NSError *error) {
            if (!succeeded) {
        ...
        [KKUtility unlikePhotoInBackground:self.photo block:^(BOOL succeeded, NSError *error) {
            if (!succeeded) {
                [button addTarget:self action:@selector(didTapLikePhotoButtonAction:) forControlEvents:UIControlEventTouchUpInside];
                [self setLikeUsers:originalLikeUsersArray];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:KKPhotoDetailsViewControllerUserLikedUnlikedPhotoNotification object:self.photo userInfo:[NSDictionary dictionaryWithObject:[NSNumber numberWithBool:liked] forKey:KKPhotoDetailsViewControllerUserLikedUnlikedPhotoNotificationUserInfoLikedKey]];
}
