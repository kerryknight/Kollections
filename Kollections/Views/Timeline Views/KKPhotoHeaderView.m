...
@interface KKPhotoHeaderView () 
@property (nonatomic, strong) UIView *containerView;
@property (nonatomic, strong) KKProfileImageView *avatarImageView;
...
@implementation KKPhotoHeaderView
...
- (id)initWithFrame:(CGRect)frame buttons:(KKPhotoHeaderButtons)otherButtons {
    ...
    if (self) {
        [KKPhotoHeaderView validateButtons:otherButtons];
        ...
        if (self.buttons & KKPhotoHeaderButtonsComment) {
            // comments button
        ...
        if (self.buttons & KKPhotoHeaderButtonsLike) {
            // like button
        ...
        if (self.buttons & KKPhotoHeaderButtonsUser) {
...
#pragma mark - KKPhotoHeaderView

- (void)setPhoto:(PFObject *)aPhoto {
    photo = aPhoto;

    // user's avatar
    PFUser *user = [self.photo objectForKey:kKKPhotoUserKey];
    PFFile *profilePictureSmall = [user objectForKey:kKKUserProfilePicSmallKey];
    [self.avatarImageView setFile:profilePictureSmall];

    NSString *authorName = [user objectForKey:kKKUserDisplayNameKey];
...
+ (void)validateButtons:(KKPhotoHeaderButtons)buttons {
    if (buttons == KKPhotoHeaderButtonsNone) {
        [NSException raise:NSInvalidArgumentException format:@"Buttons must be set before initializing KKPhotoHeaderView."];
    }
}

- (void)didTapUserButtonAction:(UIButton *)sender {
    if (delegate && [delegate respondsToSelector:@selector(photoHeaderView:didTapUserButton:user:)]) {
        [delegate photoHeaderView:self didTapUserButton:sender user:[self.photo objectForKey:kKKPhotoUserKey]];
    }
}
