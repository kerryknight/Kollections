...
- (void)addButtonWithTitle:(NSString *)title color:(NSString*)color block:(BlockPickerButtonCallback)completion atIndex:(NSInteger)index
{
//    NSLog(@"%s", __FUNCTION__);
    if (index >= 0)
    {
        [_blocks insertObject:[NSArray arrayWithObjects:
                               completion ? [[completion copy] autorelease] : [NSNull null],
                               title,
                               color,
                               nil]
                      atIndex:index];
...
{
//    NSLog(@"%s", __FUNCTION__);
    [self addButtonWithTitle:title color:@"gray" block:completion atIndex:index];
}
...
//    NSLog(@"%s", __FUNCTION__);
    if (buttonIndex >= 0 && buttonIndex < [_blocks count])
    {
        ...
        if (![obj isEqual:[NSNull null]]) {
            ...
            if (object) {
                //cast our passed in object as a dictionary
                NSDictionary *selection = (NSDictionary*)object;
                //cast our object as our callback block type and pass in our selection dict
                ((BlockPickerButtonCallback)obj)(selection);
            } else {
                ((void (^)()) obj)();//generic block
            }
        }
    }
...
//    NSLog(@"%s", __FUNCTION__);
    ...
    [self dismissWithClickedButtonIndex:buttonIndex withObject:nil animated:YES];
}
